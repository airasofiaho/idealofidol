...
function y(){
	$.ajax({
		url:"https://api.spotify.com/v1/search?q=artist:i.o.i&type=album"
		...
		$.ajax({
			...
			let image1 = response.albums.items[1].images[1].url;
			let mp3_1 = album.tracks.items[1].preview_url;
			let html = `
				<h1>${name}</h1>
				<img src="${image1}" />
				<audio controls>
...
function z(){
	$.ajax({
		...
		$.ajax({
			...
			console.log(album);
			let name = album.tracks.items[0].name;
			let image2 = response.albums.items[2].images[1].url;
			let mp3_2 = album.tracks.items[0].preview_url;
			let html = `
				<h1>${name}</h1>
				<img src="${image2}" />
				<audio controls>
...
function x(){
	$.ajax({
		...
		$.ajax({
			...
			let mp3_3 = album.tracks.items[0].preview_url;
			let html = `
				<h1>${name}</h1>
				<img src="${image1}" />
				<audio controls>
...
function w(){
	$.ajax({
		...
		$.ajax({
			...
			let mp3_4 = album.tracks.items[0].preview_url;
			let html = `
				<h1>${name}</h1>
				<img src="${image4}" />
				<audio controls>
